---
title: 基于 Mybatis Plus 的数据持久化 API 总结
authors: [xiongchun]
tags: [服务端开发,盘古开发框架,数据持久化]
date: 2018-7-15
hide_table_of_contents: false
slug: mybatis-plus-data-persistance-summary
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

[盘古开发框架](https://github.com/xiongchun/pangu-framework) 数据持久化 API 依赖 MyBatis Plus 提供。在 MyBatis 的基础上提供了强大的内置通用 Mapper CRUD 操作、支持 Lambda 表达式、内置屏蔽不通数据库方言差异的分页插件、自动填充字段值、多种主键自动生成策略、逻辑删除、乐观锁插件等。
...
> 为便于理解，本文基于本地配置的方式编写。若改为标准的Nacos配置中心模式，请参阅:[配置中心](https://xiongchun.github.io/pangu-framework/docs/advanced-guide/nacos-config-center)章节。
...
## 生成持久化所需的模版代码
基于实践经验，我们建议使用盘古代码生成器，而不是使用 Mybatis Plus 官方生成器。关于生成代码相关内容，请参阅 [《代码生成器插件》](https://xiongchun.github.io/pangu-framework/docs/code-generator) 章节，这里不再赘述。（针对每一个数据表会生成一个 `*Mapper.java` 文件和一个 `*Entity.java` 文件）
...
### 事务
单一数据源事务，直接使用 Spring 事务相关注解即可（ `@EnableTransactionManagement`、`@Transactional` ）。对于分布式事务请参阅 [《分布式事务》](https://xiongchun.github.io/pangu-framework/docs/advanced-guide/distributed-transaction) 章节。
